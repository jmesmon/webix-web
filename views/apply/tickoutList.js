    ...
    var del = function(){
        ...
        webix.confirm({
            ...
            callback:function(res){
                if(res){
                    var w = loading();
                    doPost('apply/tickout/delete', data, function(data){
                        w.close();
                        if(data.success){
                            datatable.reload();
                        }else{
                ...
                {width: 400},
                {
                    cols:[
                        ...
                        {view: "button", label: "提交", width: 65, click: function () {
                            ...
                            doIPost('apply/tickout/update', da, function(res){
                                win.close();
                                if(res.success){
                                    $$(datatableId).reload();
                                }else{
        ...
        rows: [
            {
                ...
                cols: [
                    ...
                    {view: "button", label: "分局审批", width: 70, click: function () {
                        approve(false);
                    }, permission: 'apply.tickout.approve'},
                    {view: "button", label: "九支队终审", width: 90, click: function () {
                        approve(true);
                    }, permission: 'apply.tickout.approve.final'},
                    // {view: "button", label: "删除", width: 70, click: del, permission: 'apply.tickout.delete'},
                    {},
                ]
            },
            {
                ...
                columns: [
                    ...
                    {id: "applyUnit", header: "申请单位", width: 100, sort: "string"},
                    ...
                    {id: "dogName", header: "警犬名称", width: 100, sort: "string", template: '#dogInfo.dogName#'},
                ...
                customUrl: {
                    url: webix.proxy('customProxy','/policeDog/services/apply/tickout/getList/{pageSize}/{curPage}'),
                    httpMethod: 'post',
                    datatype: 'customJson'
                },
                pager: "pagerA"
